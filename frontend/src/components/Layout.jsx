...
export default function Layout({ children }) {
  const { currentRole, currentUser, logout, resetAllData } = useApp();
  ...
  // Use real registered user info if available
  ...
  const navItems = [
    ...getNavItems(),
    { label: 'C# API Testing', path: '/integration-demo', icon: Database },
  ];

  const handleLogout = () => {
    if (window.confirm('Sign out of HireTrax?')) {
      logout();
    }
  };
    ...
    <div style={layoutStyle}>
      ...
      <aside style={sidebarStyle} className="glass-card">
        ...
        <div style={logoContainerStyle}>
          ...
          <div>
            ...
            <span style={logoSubStyle}>AI TALENT MANAGEMENT</span>
          </div>
        </div>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {navItems.map(({ label, path, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon size={17} color={active ? 'hsl(190 95% 50%)' : 'hsl(215 20% 55%)'} />
                <span style={{ fontSize: '0.88rem', fontWeight: active ? '600' : '400' }}>
                  {label}
                </span>
              </button>
            );
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <div style={sidebarFooterStyle}>
            ...
            <span style={{ fontSize: '0.78rem', color: 'hsl(215 20% 40%)' }}>
              System Online
            </span>
          </div>
        </div>
      ...
      <div style={mainAreaStyle}>
        ...
        <header style={headerStyle} className="glass-card">
          {/* Mobile menu toggle */}
          <button onClick={() => setMobileOpen(v => !v)} style={mobileToggleStyle}>
            {mobileOpen ? <X size={20} /> : <Menu size={20} />}
          </button>

          {/* Page label */}
          <div style={pageLabelStyle}>
            <span style={pageLabelTextStyle}>{meta.label} Portal</span>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              ...
              <div style={{ textAlign: 'left' }}>
                <p style={{ fontSize: '0.82rem', fontWeight: '700', color: 'white', lineHeight: 1 }}>
                  {displayName}
                </p>
                <p style={{ fontSize: '0.72rem', color: 'hsl(215 20% 55%)', lineHeight: 1.4 }}>
                  {displayEmail}
                </p>
              </div>
              <ChevronDown size={14} color="hsl(215 20% 55%)" />
            </button>
            ...
            {userMenuOpen && (
              <div style={dropdownStyle} className="glass-card">
                ...
                >
                  <LogOut size={15} color="hsl(350 89% 60%)" />
                  <span style={{ color: 'hsl(350 89% 60%)', fontSize: '0.88rem', fontWeight: '600' }}>
                    Sign Out
                  </span>
                </button>
              </div>
        ...
        {mobileOpen && (
          <div style={mobileDrawerStyle} className="glass-card">
            ...
            <button
              ...
              style={{ ...resetBtnStyle, marginTop: '16px', borderColor: 'hsl(350 89% 60% / 0.3)', color: 'hsl(350 89% 60%)' }}
            >
              <LogOut size={14} />
              <span>Sign Out</span>
            </button>
...
const sidebarStyle = {
  width: '268px', height: 'calc(100vh - 32px)',
  margin: '16px 0 16px 16px',
  display: 'flex', flexDirection: 'column',
  padding: '22px', borderRadius: '18px', flexShrink: 0,
};
...
const logoTitleStyle = {
  fontSize: '1.22rem', fontWeight: '800', lineHeight: 1,
  fontFamily: 'var(--font-header)',
};

const logoSubStyle = {
  fontSize: '0.6rem', color: 'hsl(215 20% 40%)',
  letterSpacing: '0.15em', fontWeight: '700',
};

const dividerStyle = {
  height: '1px', backgroundColor: 'hsl(217 20% 16%)',
  margin: '18px 0',
};
...
const navStyle = {
  ...
  borderRadius: '8px', color: 'hsl(215 20% 55%)',
...
const rolePillStyle = (color, bg) => ({
  ...
  background: bg, border: `1px solid ${color.replace(')', ' / 0.25)').replace('hsl(', 'hsl(')}`,
  borderRadius: '99px', padding: '7px 14px', marginBottom: '10px',
});
...
const resetBtnStyle = {
  ...
  background: 'transparent', border: '1px dashed hsl(217 20% 20%)',
  color: 'hsl(215 20% 45%)', fontSize: '0.75rem',
...
const headerStyle = {
  height: '68px', margin: '16px 16px 0 16px',
  padding: '0 24px',
...
const pageLabelStyle = { display: 'flex', alignItems: 'center' };
...
const userChipStyle = (bg, color) => ({
  ...
  background: bg,
  border: `1px solid ${color.replace(')', ' / 0.25)').replace('hsl(', 'hsl(')}`,
  borderRadius: '99px', padding: '6px 14px 6px 6px',
  cursor: 'pointer', transition: 'all 0.2s',
});

const avatarStyle = (color) => ({
  width: '28px', height: '28px', borderRadius: '50%',
  background: color.replace(')', ' / 0.2)').replace('hsl(', 'hsl('),
  color: color, fontWeight: '800', fontSize: '0.8rem',
  display: 'flex', alignItems: 'center', justifyContent: 'center',
  flexShrink: 0,
});
...
const dropdownStyle = {
  ...
  minWidth: '160px', padding: '8px', zIndex: 200,
};
