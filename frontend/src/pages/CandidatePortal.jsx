...
export default function CandidatePortal() {
  ...
  const loadAllData = async () => {
    ...
    setError('');
    try {
      // Load jobs and my applications in parallel
      const [jobsData, appsData] = await Promise.all([
        api.getJobs(),
        api.getMyApplications().catch(() => []) // safe fallback
      ]);
      setJobs(jobsData);
      ...
      // Load profile
      if (currentUser?.id) {
        const profileData = await api.getProfile(currentUser.id).catch(() => null);
        if (profileData) {
          setProfile(profileData);
          setProfileName(profileData.fullName || '');
          setProfileSkills(profileData.skills || '');
          setProfileExp(profileData.experience || '');
          setProfileSummary(profileData.professionalSummary || '');
        }

        // Load AI recommendations (only if profile has skills)
        const recData = await api.getRecommendations().catch(() => ({ recommendations: [] }));
  ...
  const handleApply = async (e) => {
    ...
    try {
      await api.createApplication({
        ...
        resumeFileName: resumeFile?.name || 'profile_resume.pdf',
        skills: profileSkills,
        experience: profileExp
      });
      setSuccessMsg(`Successfully applied to "${selectedJob.title}"!`);
      setSelectedJob(null);
      // Reload applications
      const appsData = await api.getMyApplications().catch(() => []);
      setMyApplications(appsData);
  ...
  const handleResumeChange = async (e) => {
    ...
    try {
      await api.uploadResume(currentUser.id, file);
      clearInterval(interval);
      setUploadProgress(100);
      setParsedMsg('✅ Resume uploaded successfully! AI skill detection complete.');
      // Simulate AI parsing feedback
      setTimeout(() => {
        setIsParsing(false);
        setParsedMsg('Resume linked to your profile. AI match scores updated.');
      }, 1000);
    } catch (err) {
      clearInterval(interval);
      setIsParsing(false);
      setParsedMsg('Upload failed: ' + (err.message || 'Unknown error'));
    }
  };

  // ── Helpers ──────────────────────────────────────────────────────
  const getMatchScore = (job) => {
    if (!profileSkills) return 50;
    const skills = profileSkills.toLowerCase();
    const preferred = (job.aiPreferredSkills || '').split(',').map(s => s.trim().toLowerCase());
    const matches = preferred.filter(s => s && skills.includes(s));
    return Math.min(50 + matches.length * 10, 100);
  };

  const isRecommended = (job) => getMatchScore(job) >= 70;
  const isAlreadyApplied = (jobId) => myApplications.some(a => a.jobId === jobId);
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh', flexDirection: 'column', gap: '16px' }}>
      <Loader size={40} color="hsl(263 85% 64%)" style={{ animation: 'spin 1s linear infinite' }} />
      <p style={{ color: 'hsl(var(--text-muted))' }}>Loading your portal...</p>
    </div>
  );
                ...
                return (
                  <div key={job.id} className="glass-card glass-card-interactive" style={jobCardStyle}>
                    ...
                    <div style={jobMetadataRowStyle}>
                      ...
                      <span style={metaItemStyle}><DollarSign size={14} /> {job.salaryRange}</span>
                    </div>
                    ...
                    <div style={jobCardFooterStyle}>
                      ...
                      <button 
                        ...
                        style={applyBtnStyle(alreadyApplied)}
                        disabled={alreadyApplied}
                      >
                        {alreadyApplied ? 'Applied' : 'Apply Now'} <ArrowRight size={14} style={{ marginLeft: '6px' }}/>
                      </button>
                    </div>
                  </div>
                );
                    ...
                    <div key={app.id} className="glass-card" style={appCardStyle}>
                      <div style={appCardHeaderStyle}>
                        <div>
                          ...
                          <p style={{ fontSize: '0.85rem', color: 'hsl(var(--text-muted))' }}>Applied on: {new Date(app.appliedDate).toLocaleDateString()}</p>
                        </div>
                        <div>
                      ...
                      <div style={aiReportBoxStyle}>
                        ...
                        <p style={aiReportDescStyle}>{app.aiFeedback || 'AI screening is pending or not available.'}</p>
          ...
          <div>
            ...
            <div style={profileSplitStyle}>
              ...
              <div className="glass-card" style={resumeUploadCardStyle}>
                ...
                {profile?.resumePath && !isParsing && (
                  <div style={{ marginTop: '16px', textAlign: 'center' }}>
                    <a href={`http://localhost:5027${profile.resumePath}`} target="_blank" rel="noopener noreferrer" style={{ color: 'hsl(var(--accent))', fontSize: '0.85rem', textDecoration: 'underline' }}>
              ...
              <div className="glass-card" style={profileFormCardStyle}>
                ...
                <div className="form-group">
                  ...
                  <textarea 
                    ...
                    placeholder="Briefly describe your career goals and overall expertise..."
              ...
              <form onSubmit={handleApply} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" className="form-input" required value={profileExp} onChange={(e) => setProfileExp(e.target.value)} />
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <div style={resumeAttachedStyle}>
                    ...
                    <span style={{ fontSize: '0.85rem', flexGrow: 1 }}>{resumeFile?.name || 'profile_resume.pdf'}</span>
                    <span className="badge badge-success">Attached</span>
                  </div>
                </div>

                <button type="submit" style={submitApplicationBtnStyle} disabled={applying}>
...
const applyBtnStyle = (alreadyApplied) => ({
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'center',
  gap: '8px',
  width: '100%',
  background: alreadyApplied ? 'hsl(var(--border))' : 'hsl(var(--primary))',
  color: alreadyApplied ? 'hsl(var(--text-muted))' : 'white',
  border: 'none',
  padding: '12px',
  borderRadius: 'var(--radius-sm)',
  fontWeight: '600',
  cursor: alreadyApplied ? 'not-allowed' : 'pointer',
  boxShadow: alreadyApplied ? 'none' : '0 4px 14px hsl(var(--primary) / 0.2)',
